...
async function migrateProjects() {
  try {
    ...
    console.log(`Found ${projects.length} projects without verification tokens`);
    ...
    for (const project of projects) {
      const token = generateVerificationToken(project.project_id || project._id.toString());
      ...
      console.log(`Updated project "${project.name}" (${project._id}) with token`);
      updated++;
    }
    
    console.log(`Successfully updated ${updated} projects with verification tokens`);
  } catch (error) {
