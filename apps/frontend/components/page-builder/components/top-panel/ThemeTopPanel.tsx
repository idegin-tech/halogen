...
export default function ThemeTopPanel({show, onHide}:{show: boolean, onHide: () => void}) {
    ...
    const handleVariableChange = (variableId: string, value: string) => {
        console.log(`Updating variable ${variableId} with value ${value}`);
    ...
    }, [selectedVariableSetId]); // Run when the selected set changes
    ...
    const breadcrumbs = useMemo(() => {
        ...
        if (selectedVariableSetId) {
            const variableSet = state.variableSets.find(set => set.id === selectedVariableSetId);
            if (variableSet) {
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4 mt-4">
                    {colorVariables.map(variable => (
                        <div key={variable.id} className="flex items-center gap-2 p-2 border rounded-md hover:border-primary transition-colors">
                            <div 
                                ...
                                onClick={() => toggleColorPicker(variable.id)}
                            />
                            <div className="flex-1">
                                <p className="font-medium">{variable.name}</p>
                                <p className="text-xs text-muted-foreground">{variable.primaryValue}</p>
                            </div>
                            <Popover open={colorPickerVisible[variable.id]} onOpenChange={() => toggleColorPicker(variable.id)}>
    ...
    return (
        <TopPanelContainer
            ...
            subPageHeading={selectedVariableSetId ? 
                state.variableSets.find(set => set.id === selectedVariableSetId)?.name :
                "Select a theme set"}
